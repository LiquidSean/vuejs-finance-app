  ...
  methods: {
    ...
    deleteItem(item) {
      ...
      if (shouldDelete) {
        ...
        state.db.collection('transactions').doc(item.id).delete();
      }
    },
    saveItem() {
      const state = store;
      ...
      if (this.editedIndex > -1) {
          ...
          state.db.collection('users').doc(state.currentUser.uid).collection('transactions').doc(this.editedItem.id.toString()).update(tranObject)
          .then((result) => {
            this.close();
          });
        } else {
          state.db.collection('users').doc(state.currentUser.uid).collection('transactions').add(tranObject)
            .then((doc) => {
